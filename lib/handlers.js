...
function createHandler(opts) {
    ...
    // node >=16 has dropped the async support, below code is compatible without async lib
    app.get('/download_new', function (req, res) {
        var stream = new JSZip(),
        writer = new ZipWriter(stream, process.cwd()),
            coverageObject = core.getCoverageObject() || {},
            collector = new Collector(),
            baseDir = process.cwd(),
        ...
        stream.generateAsync({ type: "base64" }).then((base64) => {
            let zip = Buffer.from(base64, "base64");
            res.writeHead(200, {
                "Content-Type": "application/zip",
                "Content-disposition": `attachment; filename=coverage.zip`,
            });
            res.end(zip);
        });
    });
