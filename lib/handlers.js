...
function createHandler(opts) {
    ...
    app.get('/download', function (req, res) {
        let stream = new AdmZip();
        const coverageObject = core.getCoverageObject() || {};
        ...
        stream.addFile('coverage.json',Buffer.from(JSON.stringify(coverageObject, undefined, 4), "utf8") );
        const lcov = Report.create('lcovonly',{ dir: baseDir })
        const html= Report.create('html',{ dir: path.join(baseDir, 'lcov-report') })
        html.writeReport(collector);
        lcov.writeReport(collector);
        html.on('done', function () {
            stream.addLocalFile(baseDir+'/lcov.info','');
            stream.addLocalFolder(baseDir+'/lcov-report','lcov-report');
            const output ="coverage.zip";
            fs.writeFileSync(output, stream.toBuffer());
            res.download(output);
        });
    });
